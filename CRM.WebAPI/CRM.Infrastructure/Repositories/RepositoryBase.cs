...
public abstract class RepositoryBase
{
    ...
    protected RepositoryBase(IConfiguration configuration) // инициализируем
    {
        ...
        _connectionString = configuration.GetConnectionString("PostgreConnectionString"); //идем в кфг файл и берем оттуда подключение
    }
        ...
        where T : class where TCreator : ICreator<T>, new()
    {
        ...
        await using (var command = new NpgsqlCommand(sql, connection)) //выполняем команду (она нам пришла аргументом при вызове метода из конкретного репозитория
        {
